package ycanvas.feathers.components
{
	...
	public class Partition implements IPartitionStarling
	{
		...
		private function get url():String
		{
			...
			//var server:uint = Math.abs(x + y) % 3 + 1;
			//var result:String = "http://otile" + server + ".mqcdn.com/tiles/1.0.0/osm/" 
			//	+ level + "/" + x + "/" + y + ".jpg";
			
			var servers:Array = [ 
				"a", "b", "c", "d" 
			];
			var result:String = "http://" + servers[Math.round(Math.random() * 3)] + ".tiles.mapbox.com/v3/hannuval.map-azrlxmmu/" 
				+ level + "/" + x + "/" + y + ".jpg";
		...
		public function dispose():void
		{
			...
			if (content)
			{
				var texture:Texture = (content as Image).texture;
